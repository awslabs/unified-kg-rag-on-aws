...
class TestCoverage:
    def test_full_match(self, evaluator: GraphAwareEvaluator) -> None:
        p, r, f1, m = evaluator._coverage(["Alice", "Acme"], "Alice and Acme")
        assert (p, r, f1, m) == (1.0, 1.0, 1.0, 2)

    def test_partial_match_case_insensitive(
        self, evaluator: GraphAwareEvaluator
    ) -> None:
        p, r, f1, m = evaluator._coverage(["Alice", "Bob"], "ALICE only")
        assert m == 1
        assert r == 0.5

    def test_empty_expected_returns_none(self, evaluator: GraphAwareEvaluator) -> None:
        # None scores so an absent dimension is excluded from the overall score.
        assert evaluator._coverage([], "anything") == (None, None, None, 0)
    ...
    def test_no_false_substring_match(self, evaluator: GraphAwareEvaluator) -> None:
        ...
        _, recall, _, matched = evaluator._coverage(["AI"], "the airport is busy")
        assert matched == 0 and recall == 0.0

    def test_word_match_succeeds(self, evaluator: GraphAwareEvaluator) -> None:
        _, recall, _, matched = evaluator._coverage(["AI"], "we use AI models")
        assert matched == 1 and recall == 1.0

    def test_multiword_phrase_requires_contiguous_order(
        self, evaluator: GraphAwareEvaluator
    ) -> None:
        assert evaluator._coverage(["works at"], "she works at acme")[3] == 1
        # Same words, wrong order -> no match.
        assert evaluator._coverage(["at works"], "she works at acme")[3] == 0
    ...
    def test_punctuation_is_stripped(self, evaluator: GraphAwareEvaluator) -> None:
        ...
        assert evaluator._coverage(["Acme"], "owned by Acme, Inc.")[3] == 1
...
class TestEvaluateSingle:
    ...
    def test_emits_six_graph_metrics(self, evaluator: GraphAwareEvaluator) -> None:
    ...
    def test_missing_expectations_score_zero(
        ...
        entity_recall = next(
            ...
            if m.metric_type == EvaluationMetricType.ENTITY_RECALL
        )
        assert entity_recall == 0.0
