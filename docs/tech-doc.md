...
- **라이브 그래프 cross-run merge(Neptune read-merge-write) 미적용**: 증분 upsert(`_set_properties_on_traversal`)는 멱등하게 *덮어쓰기*합니다 — 같은 자연키의 엔티티/관계를 다시 upsert하면 description·`text_unit_ids`가 이번 델타 값으로 교체됩니다. 한 실행(run) 내 병합은 `graph_resolver`+`merge/merger.py`가 정확히 처리하지만, *이전 실행이 그래프에 이미 누적한 상태*와 델타를 union하려면 upsert 전에 기존 정점/엣지를 읽어 `merge_entities`/`merge_relationships`로 합치는 read-merge-write가 필요합니다. 이는 실제 Neptune 왕복이 있어야 검증 가능하므로(인메모리 fake로 불가) 현재 미적용 — 향후 `IndexingManager.index_delta` 경로에 read-merge 단계를 추가하면 됩니다. (변경 문서 재처리 시 `prune_changed`가 이전 아티팩트를 먼저 제거하므로 *고아*는 생기지 않으나, 서로 다른 문서가 공유하는 엔티티의 설명 누적은 마지막 델타 값으로 수렴함.)
- **`DocStatusPort` 주입 vs 인-라인 생성**: `DocumentLoadingStage`/`IndexingStage`가 `DynamoDBDocStatusStore`를 인-라인으로 lazy import·생성합니다. 헥사고날 원칙상 스테이지/파이프라인 생성자에 `DocStatusPort`를 주입하고 구체 어댑터는 오케스트레이션 레이어에서 한 번만 생성해 내려주는 것이 더 깔끔합니다(매 실행마다 세션/스토어 재생성 회피).
- ~~**community report 삭제 lineage**~~ (해결): community report도 자신이 요약하는 커뮤니티의 멤버 text-unit을 통해 문서에 귀속되도록 `community_report_ids` lineage를 추가했고, `delete_documents`가 community-reports 인덱스도 정리합니다. 문서 삭제 시 해당 문서의 *독점* 커뮤니티 리포트가 제거됩니다(공유분은 보존).
